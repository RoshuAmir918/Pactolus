...
export function WorkspaceSidebar() {
  const [views, setViews] = useState<ClientView[]>(() => {
    ...
    try {
      const stored = window.localStorage.getItem(VIEWS_STORAGE_KEY);
      if (!stored) {
        const firstOrg = DEMO_ORGS[0];
        return [
          {
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.localStorage.setItem(VIEWS_STORAGE_KEY, payload);
    } catch {
                    ...
                    <Popover open={addViewOpen} onOpenChange={setAddViewOpen}>
                      <PopoverTrigger asChild>
                        <Button
                          ...
                          size="sm"
                          className={cn(
                            "mt-2 flex w-full items-center justify-start gap-2 text-xs",
                            addViewOpen &&
